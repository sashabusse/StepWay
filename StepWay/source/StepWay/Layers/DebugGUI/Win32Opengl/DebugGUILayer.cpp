...
namespace StepWay
{
	...
	void DebugGUILayer::OnMouseButtonPress(Event& e)
	{
		...
		Input::MouseKeyCode keyCode = pPressEvent->GetKeyCode();
		int button = 0;
		if (keyCode == MouseKeyCode::L_BUTTON) { button = 0; }
		if (keyCode == MouseKeyCode::R_BUTTON) { button = 1; }
		if (keyCode == MouseKeyCode::MID_BUTTON) { button = 2; }
		//if (keyCode == WM_XBUTTONDOWN || keyCode == WM_XBUTTONDBLCLK) { button = (GET_XBUTTON_WPARAM(wParam) == XBUTTON1) ? 3 : 4; }
		io.MouseDown[button] = true;
		if (::GetCapture() == NULL)
			::SetCapture(m_pWindow->GetHWND());
	}
	...
	void DebugGUILayer::OnMouseButtonRelease(Event& e)
	{
		...
		Input::MouseKeyCode keyCode = pReleaseEvent->GetKeyCode();
