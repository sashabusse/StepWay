...
bool SandboxApp::ImplSetUp()
{
	...
	tabUniform.AddWidget(std::make_shared<DbgSliderf>("b_temp", &b_temp, 0.0, 100.0));
	...
	m_shader->SetUpFromFile("Resource\\GLShaders\\HeatEquationSolvation\\DoubleAxis\\DrawingVertex.hlsl",
		"Resource\\GLShaders\\HeatEquationSolvation\\DoubleAxis\\DrawingFragment.hlsl");
	//----------------------------------
	...
	for (int i = 0; i < 2; i++)
	{
		...
		texture_buffer[i]->SetUp(N, N, Texture::PixelFormat::R32_F, NULL);
		texture_buffer[i]->SetToTexUnit(i + 1);
		texture_buffer[i]->SetToImgUnit(i + 1);
	}
	texture_buffer[0]->LoadData(N, N, Texture::PixelFormat::R32_F, starting_condition());
	texture_buffer[1]->LoadData(N, N, Texture::PixelFormat::R32_F, starting_condition());

	compute_shader = std::shared_ptr<Shader>(Shader::Create(GAPI_TYPE::OPENGL));
	compute_shader->SetUpAsComputeShader("Resource\\GLShaders\\HeatEquationSolvation\\DoubleAxis\\Compute.hlsl");
	...
	compute_shader->SetUniform("b_temp", b_temp);
